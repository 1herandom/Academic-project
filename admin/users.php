...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reset_password'])) {
    ...
    if (in_array($table, ['admins', 'teachers', 'students'], true)) {
        ...
        $_SESSION['new_user_email'] = $targetUser['email'] ?? '';
        $emailResult = null;
...
// ── Dismiss Reset Banner ────────────────────────────────────────────────────
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['dismiss_reset_banner'])) {
    if (hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'] ?? '')) {
        unset(
            $_SESSION['last_reset_user_id'],
            $_SESSION['last_reset_table'],
            $_SESSION['last_reset_password'],
            $_SESSION['last_reset_email'],
            $_SESSION['last_reset_name'],
            $_SESSION['last_reset_login_email']
        );
    }
    redirect('/admin/users.php');
}
...
if ($searchQuery !== '') {
    $where .= " AND (first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR institutional_id LIKE ?)";
    $like = '%' . $searchQuery . '%';
...
$sql = "SELECT *" . $sqlBase . $where . " ORDER BY created_at DESC LIMIT " . (int)$limit . " OFFSET " . (int)$offset;
