...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_assignment'])) {
    ...
    if (!empty($_FILES['brief_file']['tmp_name'])) {
        ...
        if (!is_dir($absBriefDir)) {
            mkdir($absBriefDir, 0775, true);
            @chmod($absBriefDir, 0775);
        }
        if (!is_writable($absBriefDir)) {
            @chmod($absBriefDir, 0775);
        }
        if (!is_writable($absBriefDir)) {
            flash_set('error', 'Upload directory is not writable. Please check server permissions.');
            redirect('/teacher/add_assignment.php');
        }
        
        $targetFile = $absBriefDir . $name;
        if (move_uploaded_file($_FILES['brief_file']['tmp_name'], $targetFile)) {
            $briefFilePath = $briefDir . $name;
        } else {
            $error = $_FILES['brief_file']['error'];
            $msg = 'Failed to upload brief file.';
            if ($error !== UPLOAD_ERR_OK) $msg .= ' (PHP Error: ' . $error . ')';
            flash_set('error', $msg);
            redirect('/teacher/add_assignment.php');
        }
    }
