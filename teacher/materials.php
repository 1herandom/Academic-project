...
try {
    ...
    $materials = [];
    flash_set('error', 'Unable to load materials: ' . $e->getMessage());
}
?>
...
.viewer-backdrop.open { opacity: 1; visibility: visible; }
.viewer-box {
...
.viewer-backdrop.open .viewer-box { transform: translateY(0); }
.viewer-header {
    display: flex;
    align-items: center;
...
.viewer-title { font-weight: 700; font-size: 1rem; color: var(--text-main); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.viewer-meta  { font-size: 0.8rem; color: var(--text-muted); margin-top: 2px; }
.viewer-close {
    background: none;
    border: 1px solid var(--border-color);
...
.viewer-close:hover { background: var(--surface-hover); color: var(--text-main); }
.viewer-body { flex: 1; overflow: auto; display: flex; flex-direction: column; min-height: 0; }
.viewer-iframe { width: 100%; flex: 1; border: none; min-height: 600px; background: #fff; }
.viewer-unsupported {
    padding: 3rem 2rem; text-align: center; color: var(--text-muted);
    display: flex; flex-direction: column; align-items: center; gap: 1rem;
}
.viewer-unsupported svg { width: 48px; height: 48px; opacity: 0.5; }
.viewer-footer {
...
.btn.view-btn:hover { background: rgba(104,186,127,0.22); }
</style>

<div class="page-header-layout">
    <div>
        <h1 class="page-title">Materials</h1>
        <p class="muted page-subtitle">Manage and preview your learning resources.</p>
    </div>
    <div>
        <a href="<?= APP_BASE_URL ?>/teacher/add_material.php" class="btn">Add Material</a>
    </div>
</div>
            ...
            <tbody>
                ...
                <?php foreach ($materials as $m): ?>
                    ...
                    <tr>
                        ...
                        <td><span class="pill muted"><?= esc($fileType) ?></span></td>
                        <td style="display:flex; gap:6px; flex-wrap:wrap;">
                            ...
                            <button class="btn sm view-btn"
                                onclick="openViewer(
                                    ...
                                    <?= htmlspecialchars(json_encode($m['file_path'] ? $viewUrl : ''), ENT_QUOTES) ?>,
                                    <?= htmlspecialchars(json_encode($m['video_link'] ?? ''), ENT_QUOTES) ?>,
                                    <?= htmlspecialchars(json_encode($m['course_code'] . ' · ' . $fileType), ENT_QUOTES) ?>
                                )">
...
document.getElementById('viewerBackdrop').addEventListener('click', e => { if (e.target === document.getElementById('viewerBackdrop')) closeViewer(); });
