...
function bindAttendanceCounters() {
    ...
    const updateCounts = () => {
        const presentChecked = container.querySelectorAll(
            'input[type="checkbox"][data-status="present"]:checked'
        ).length;

        const absentChecked = container.querySelectorAll(
            'input[type="checkbox"][data-status="absent"]:checked'
        ).length;
    ...
    container.addEventListener('change', (event) => {
        if (event.target.matches('input[type="checkbox"]')) {
            ...
            if (
                row &&
                event.target.dataset.status === 'present' &&
            ...
            if (
                row &&
                event.target.dataset.status === 'absent' &&
                event.target.checked
            ) {
    ...
    document
        .querySelector('[data-mark-all-present]')
        ?.addEventListener('click', () => {
            container
                .querySelectorAll('input[data-status="present"]')
                .forEach((cb) => (cb.checked = true));

            container
                .querySelectorAll('input[data-status="absent"]')
...
function deadlineLockdown() {
    ...
    const timer = () => {
        ...
        if (now >= deadline + 60000) {
            ...
            if (note)
                note.textContent =
                    'The deadline has passed. Submission is closed.';
