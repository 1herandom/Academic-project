...
function bindAttendanceCounters() {
  ...
  const updateCounts = () => {
    // Works with both radio and checkbox inputs
    const presentChecked = container.querySelectorAll('input[data-status="present"]:checked').length;
    const absentChecked  = container.querySelectorAll('input[data-status="absent"]:checked').length;
  ...
  // Radio buttons are mutually exclusive by name — no extra JS needed.
  // The change listener just keeps the counters live.
  ...
  // Mark All Present: select the Present radio in every student row.
  document.querySelector('[data-mark-all-present]')?.addEventListener('click', () => {
    container.querySelectorAll('[data-student-row]').forEach(row => {
      const presentRadio = row.querySelector('input[data-status="present"]');
      if (presentRadio) {
        presentRadio.checked = true;
        // Uncheck absent explicitly for checkbox fallback compatibility
        const absentInput = row.querySelector('input[data-status="absent"]');
...
function bindPasswordToggles() {
  document.querySelectorAll('[data-pw-toggle]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (eyeOpen)   eyeOpen.style.display   = isText ? 'inline' : 'none';
      if (eyeClosed) eyeClosed.style.display = isText ? 'none'  : 'inline';
    });
  });
}
